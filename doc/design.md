...
│   │   ├── lifo_event_buffer.h / .cpp    🆕 LIFO 事件栈（jAER AEStack 式回溯）
...
│   ├── cv/                      # 计算机视觉与运动分析
│   │   ├── noise_filter.h / .cpp          # 🆕 多模式（BAF/STCF/Refractory/DWF/AgePolarity/Harmonic/Repetitious/SpatialBP）
│   │   ├── hot_pixel_filter.h / .cpp      🆕 热像素过滤（学习+查表+FPN 概率校正）
│   │   ├── orientation_filter.h / .cpp    🆕 4 朝向边缘检测（参考 AbstractOrientationFilter）
│   │   ├── direction_selective_filter.h / .cpp  🆕 8 方向运动估计（参考 AbstractDirectionSelectiveFilter）
│   │   ├── sparse_optical_flow.h / .cpp   # 🆕 三模式（LocalPlanes/LucasKanade/BlockMatch）
│   │   ├── dense_optical_flow.h / .cpp    # 密集光流
│   │   ├── cluster_optical_flow.h / .cpp  🆕 聚类光流场（参考 ClusterBasedOpticalFlow）
│   │   ├── blob_detector.h / .cpp         # 🆕 连通域 + Histogram2DFilter 背景建模
│   │   ├── object_tracker.h / .cpp        # 🆕 四模式（RCT/Median/Kalman/MultiHypothesis）
│   │   ├── corner_detector.h / .cpp       # 🆕 三模式（EndStopped/TypeCoincidence/Harris）
│   │   ├── cluster_interface.h            # 🆕 聚类抽象基类（参考 ClusterInterface）
│   │   ├── cluster_path_point.h           # 🆕 聚类轨迹点（位置/速度/时间戳）
│   │   ├── line_segment_detector.h / .cpp # 🆕 ELiSeD 线段检测（参考 ELiSeD）
│   │   ├── hough_line_tracker.h / .cpp    # 🆕 霍夫直线跟踪（参考 HoughLineTracker）
│   │   ├── hough_circle_tracker.h / .cpp  # 🆕 霍夫圆跟踪（参考 HoughCircleTracker）
│   │   ├── hinge_line_tracker.h / .cpp    # 🆕 铰链线跟踪（双线段铰链）
│   │   ├── orientation_cluster.h / .cpp   # 🆕 方向共识过滤（朝向一致聚类）
│   │   ├── circle_tracker.h / .cpp        # 🆕 圆环跟踪（粒子滤波）
│   │   ├── cluster_lif.h / .cpp          # 🆕 LIF 神经元网格聚类（参考 ClusterBubbles）
│   │   ├── background_mask_filter.h / .cpp # 🆕 背景掩码学习（运动/静止分离）
│   │   ├── perspective_undistort.h / .cpp  # 🆕 透视去畸变（参考 SingleCameraCalibration LUT）
│   │   ├── trigger_synced_filter.h / .cpp # 🆕 触发同步过滤（仅保留触发窗口事件）
│   │   ├── bandpass_filter.h / .cpp       # 🆕 IIR 带通滤波（频率域事件过滤）
│   │   ├── optical_gyro.h / .cpp          # 🆕 电子稳定 EIS（事件陀螺仪，参考 OpticalFlowGyroTracker）
│   │   ├── ultra_slow_motion.h / .cpp     # 超高速等效回放（时间膨胀）
│   │   ├── xyt_visualizer.h / .cpp        # XYT 3D 事件点云数据层（替代 Temporal Plot）
│   │   ├── time_surface.h / .cpp         # Time Surface 窗口
│   │   └── overlay.h / .cpp              # 算法结果可视化叠加
│   ├── analytics/              # 分析模块
│   │   ├── active_marker.h / .cpp          # 主动标记跟踪（调制光方案）
│   │   ├── event_to_video.h / .cpp         # 事件→灰度重建（E2VID 神经网络）
│   │   ├── flow_statistics.h / .cpp        🆕 光流评估（PE / EPE / 端点误差）
│   │   ├── isi_analyzer.h / .cpp           🆕 ISI 直方图分析（像素间事件间隔分布）
│   │   ├── particle_counter.h / .cpp       🆕 通用颗粒计数器（参考 jAER ParticleCounter）
│   │   ├── auto_bias_controller.h / .cpp   🆕 自适应 Bias 控制（事件率反馈 PID 调节，借鉴 AutoExposureController 思路）
│   │   └── freq_detector.h / .cpp         🆕 闪烁光源频率检测（参考 freq_analyzer 热图+FFT）
│   ├── calibration/            # 相机标定
│   │   └── intrinsic.h / .cpp              # 内参标定（含事件去畸变 LUT）
│   └── tests/                  # 🆕 自研测试基建
│       ├── noise_tester.h / .cpp            # 降噪评测框架（注入泊松+漏噪声，统计 TP/FP/TN/FN）
...
> 注：`algo/analytics/event_to_video` 提供三种重建路径——两种非 DL 路径（InteractingMaps 松弛迭代、BardowVariational 变分优化，均无外部依赖）与 DL 路径（E2VID 神经网络，可选）。全部纯事件 (x,y,p,t) 输入，详见 4.4.2。DL 路径届时从 [rpg_e2vid](https://github.com/uzh-rpg/rpg_e2vid) 移植。
...
| `ClusterBasedOpticalFlow` ★ | 基于 RCT 速度场的网格化光流（带通滤波） | ✅ 移植 | `algo/cv/cluster_optical_flow.h` |
...
| `minliu/OpenCVFlow` | OpenCV Farneback 稠密光流（事件累积帧间） | ✅ 移植 | `algo/cv/dense_optical_flow.h` |
...
| `eyetracker/EyeTracker` | 圆环模型瞳孔跟踪 | ⚠️ 仅圆环模型通用 | `algo/cv/circle_tracker.h` |
...
> **总计**：自研 53 个算法模块 + 封装复用 openEB 27 项能力；openEB 全部 89 项内置功能（见 1.5 节）均需在 GUI 中可访问。
...
|------|----------|-------------|------|
| **BAF**（背景活动过滤） | Delbruck 2008：当前事件在 3×3 邻域过去 `dt` 内有事件则放行 | `BackgroundActivityFilter` | `dt_us`：int，`[1000, 100000]`，默认 `25000`；`subsample_by`：int，`[0, 4]`，默认 `0` |
| **STCF**（时空相关滤波，主力） | Guo & Delbruck 2022 T-PAMI：3×3 邻域至少 N 个相关事件，可选极性匹配 + 散粒噪声检测 | `SpatioTemporalCorrelationFilter` | `correlation_time_s`：float，`[0.001, 0.1]`，默认 `0.025`；`min_neighbors`：int，`[1, 8]`，默认 `2`；`require_polarity_match`：bool，默认 `true`；`allow_coincidence`：bool，默认 `false` |
| **Refractory**（不应期） | 逐像素不应期：同像素间隔 > 阈值才放行，去除高频热噪 | `RefractoryFilter` | `refractory_period_us`：int，`[100, 100000]`，默认 `1000` |
| **DWF**（双窗口） | 双 FIFO（信号窗 + 噪声窗），内存与分辨率无关 | `DoubleWindowFilter` | `signal_window_us`：int，`[1000, 50000]`，默认 `5000`；`noise_window_us`：int，`[10000, 500000]`，默认 `50000` |
| **AgePolarity**（软评分） | age = 1 - dt/tau，加权评分 ≥ 阈值放行 | `AgePolarityDenoiser` | `tau_us`：int，`[1000, 100000]`，默认 `10000`；`age_threshold`：float，`[0, 1]`，默认 `0.5` |
| **Harmonic**（谐波） | 全局谐波振荡器抑制 50/60 Hz 灯光闪烁 | `HarmonicFilter` | `line_freq_hz`：枚举（50/60），默认 `50`；`notch_q`：float，`[1, 50]`，默认 `10` |
| **Repetitious**（重复过滤） | 滤除周期性重复事件（屏幕闪烁等） | `RepetitiousFilter` | `period_us`：int，`[1000, 1000000]`，默认 `20000`；`tolerance_us`：int，`[100, 10000]`，默认 `1000` |
| **SpatialBP**（空间带通） | 中心/外围时间戳图差分，小目标增强 | `SpatialBandpassFilter` | `center_radius_px`：int，`[1, 10]`，默认 `2`；`surround_radius_px`：int，`[5, 30]`，默认 `10` |
...
借鉴 jAER `AbstractOrientationFilter`（见 1.6.3），将每个事件归类为 4 朝向（0°/45°/90°/135°）之一，输出朝向伪彩图。

| 子模块 | 方案 |
|--------|------|
| 朝向计算 | 3×3 邻域最近事件时间戳矩阵 → 主特征向量方向 → 量化到 4 朝向 |
| 朝向渲染 | 4 朝向映射为 4 种颜色叠加到主显示帧 |

**参数与合法范围**：`orientations`：枚举（4），固定；`time_window_us`：int，`[1000, 50000]`，默认 `10000`；`min_neighbors`：int，`[1, 8]`，默认 `2`；`color_map`：枚举（Fixed4 / HSV）

#### 4.3.8 🆕 方向选择性滤波 (DirectionSelectiveFilter) — 叠加型

借鉴 jAER `AbstractDirectionSelectiveFilter`（见 1.6.3），8 方向运动估计（上/下/左/右/对角），输出运动方向伪彩图。可选全局平移/旋转/膨胀模式。

| 子模块 | 方案 |
|--------|------|
| 8 方向计算 | 8 邻域时间戳差分最大方向即为运动方向 |
| 全局模式 | 检测整帧统一方向（平移/旋转/膨胀），输出场景运动类型 |

**参数与合法范围**：`directions`：枚举（8），固定；`time_window_us`：int，`[1000, 50000]`，默认 `10000`；`enable_global_mode`：bool，默认 `true`
...
| **ClusterOF**（聚类光流场） | 跟踪聚类质心轨迹，估计每簇光流 | `ClusterBasedOpticalFlow` | 复用 4.3.11 跟踪参数；`min_track_len`：int，`[3, 100]`，默认 `5` |
...
借鉴 jAER `HoughCircleTracker`（见 1.6.2），事件流累积帧 → 霍夫圆变换 → 圆检测与跟踪。亦支持粒子滤波圆环跟踪模式（对应 `circle_tracker.h`）。
...
借鉴 jAER `OpticalFlowGyroTracker`（见 1.6.5），估计全局相机运动（平移 + 旋转），反向补偿事件坐标，实现电子稳定。

**参数与合法范围**：`stabilization_strength`：float，`[0, 1]`，默认 `1`；`smoothing_window_ms`：float，`[10, 1000]`，默认 `100`
...
| **E2VID**（DL，可选） | E2VID / SSL-E2VID 神经网络推理，从事件流重建灰度帧。届时需从 [rpg_e2vid](https://github.com/uzh-rpg/rpg_e2vid) 移植 | — | ONNX Runtime 或 libtorch |
...
- `e2vid_inference`（DL，可选）：模型加载 + 滑动窗口推理 + 帧率控制
...
| **Algorithm** | Noise Filter, Optical Flow (Sparse/Dense), Blob Detect, Object Tracker, Corner Detect, Ultra Slow Motion, XYT 3D, **Time Surface**, Active Marker, Event→Video |
...
| **独立窗口型 (Standalone)** | 在新窗口独立显示 | Time Surface、XYT 可视化、事件重建（EventToVideo）、频率检测（FreqDetector） | 弹出独立子窗口 |
...
- [ ] `event.h` / `event_packet.h`：事件 POD 与零拷贝包
- [ ] `event_buffer.h` / `lifo_event_buffer.h`：环形缓冲 + LIFO 栈
- [ ] `frame_generator.h` / `dvs_framer.h`：帧生成与事件分帧（纯事件 ON/OFF 计数）
- [ ] `freme.h`：FREME 频率表示模板
- [ ] `data_loader.h`：HDF5/RAW 数据加载（内存映射加速）
- [ ] `event_rate_estimator.h` / `performance_meter.h` / `time_limiter.h`：性能监测
- [ ] `kalman_filter.h` / `kmeans.h` / `particle_filter.h` / `periodic_spline.h`：滤波/聚类/平滑
- [ ] `histogram_ring_buffer.h` / `lif_integrator.h`：直方图与 LIF 神经元
- [ ] `filter/`：IIR 低通/高通/带通/角度/中位数滤波器集

### Phase 7：基础噪声过滤与运动分析 (algo/cv/ 8 个，对应 4.3.5–4.3.12)
借鉴 jAER `filter/`、`label/`、`tracking/`、`projects/rbodo/`：
- [ ] 4.3.5 噪声过滤（8 模式：BAF/STCF/Refractory/DWF/AgePolarity/Harmonic/Repetitious/SpatialBP）
- [ ] 4.3.6 热像素过滤（学习 + 查表 + FPN 概率校正）
- [ ] 4.3.7 朝向边缘检测（4 朝向，`AbstractOrientationFilter`）
- [ ] 4.3.8 方向选择性滤波（8 方向 + 全局模式，`AbstractDirectionSelectiveFilter`）
- [ ] 4.3.9 光流估计（4 模式：LocalPlanes/LucasKanade/BlockMatch/ClusterOF）
- [ ] 4.3.10 团块检测（连通域 + Histogram2DFilter 背景建模）
- [ ] 4.3.11 事件级目标跟踪（4 模式：RCT/Median/Kalman/MultiHypothesis）
- [ ] 4.3.12 角点检测（3 模式：EndStopped/TypeCoincidence/Harris）
...
- [ ] 4.3.21 触发同步过滤
- [ ] 4.3.22 IIR 带通滤波
- [ ] 4.3.23 电子稳定 EIS（OpticalGyro）

### Phase 9：显示工具 (algo/cv/ 4 个，对应 4.3.24–4.3.27)
- [ ] 4.3.24 超高速等效回放（时间膨胀，≥200,000 fps）
- [ ] 4.3.25 XYT 3D 事件点云（独立窗口，替代 Temporal Plot）
- [ ] 4.3.26 可视化叠加（Overlay）
- [ ] 4.3.27 Time Surface 窗口（独立窗口，openEB `TimeSurfaceProcessor` 封装）

### Phase 10：分析模块 (algo/analytics/ 7 个)
借鉴 jAER `DvsFramer`、`particlecounter`、`freq_analyzer` 等：
- [ ] 4.4.1 主动标记跟踪（滑动窗口聚类 + 事件数颜色映射）
- [ ] 4.4.2 事件→灰度图像重建（非 DL：BardowVariational 默认 / InteractingMaps / DL：E2VID 可选）
- [ ] 4.4.3 光流评估（EPE/PE/角度误差）
- [ ] 4.4.4 ISI 直方图分析
- [ ] 4.4.5 通用颗粒计数器
- [ ] 4.4.6 自适应 Bias 控制（PID 反馈）
- [ ] 4.4.7 闪烁光源频率检测（热图 + FFT，参考 `freq_analyzer`）

### Phase 11：标定模块 (algo/calibration/ 1 个)
- [ ] 4.5.1 内参标定（含事件去畸变 LUT 预计算，借鉴 jAER `SingleCameraCalibration`）

### Phase 12：测试基建 (algo/tests/ 2 个)
借鉴 jAER `NoiseTesterFilter`：
- [ ] 4.6.1 降噪评测框架（注入泊松 + 漏噪声，统计 TP/FP/TN/FN）
- [ ] 4.6.2 信号/噪声标注事件类型

### Phase 13：完善
- [ ] 3D XYT 事件点云窗口（替代 Temporal Plot，VBO+GLSL）
