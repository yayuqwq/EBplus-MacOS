...
| BLD-011 | Runtime integration | HAL plugin discovery | Wired into application | Requires verification | In progress | Repository-local `MV_HAL_PLUGIN_PATH`、`MV_HAL_PLUGIN_SEARCH_MODE=PLUGIN_PATH_ONLY` 和无设备 `--short`/`--system` discovery smoke 已通过 | 使用真实相机验证 enumeration、open、stream、facilities、shutdown 和 reconnect | M2/M6 | Discovery stack 已进入空设备路径；真实相机和 CenturyArks 支持未验证 |
