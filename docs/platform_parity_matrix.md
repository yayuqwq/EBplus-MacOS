...
| BLD-012 | Runtime integration | HDF5 plugin discovery | Wired into application | Requires verification | In progress | Repository-local `HDF5_PLUGIN_PATH` 下 RAW→HDF5、ECF 回读及外部 `h5dump` 解码已通过。2026-07-26 repository-local HDF5 ECF plugin path successfully supported CLI reads of fresh <code>.hdf5</code>/<code>.h5</code> working copies and build-tree EBplus GUI HDF5/H5 playback. Both CLI fixtures reported ECF, 95,871 us, 521,252 CD events, timestamps 0–95,871. GUI export/conversion and DAT remain unverified；见 [HDF5/H5 validation](macos_hdf5_h5_generic_offline_validation.md) | 验证 EBplus GUI 的 HDF5 转换和导出路径，并扩展 HDF5/H5 file corpus | M2/M5/M7 | OpenEB CLI 路径与 EBplus GUI HDF5/H5 playback 已获 bounded evidence；转换和导出仍 pending |
...
| GUI-006 | GUI | Settings sidebar and panels | Wired into application | Requires verification | In progress | 用户观察到 sidebar 与主要 idle panels；见 [M4A validation](macos_milestone_4a_validation.md) | 逐 panel 交互和状态切换 | M4/M6/M7 | 未验证各 panel interaction、active-group persistence 或 camera/facility/algorithm panels |
...
| RAW-017 | RAW | File-to-file switch | Implemented in source | Requires verification | Not started | 再次调用 <code>PlaybackController::open_file()</code> 会经 <code>CameraController::connect_file()</code> teardown 旧 source。2026-07-24 same-file reopen and different-path same-content RAW switch both succeeded post-fix and automatically resumed playback; Recent reopen also auto-resumed. different geometry and broader failure rollback remain unverified；见 [lifecycle validation](macos_raw_file_lifecycle_validation.md)。2026-07-26 HDF5→H5 same-content/different-extension file switch succeeded and autoplay continued post-fix. Different geometry remains unverified；见 [HDF5/H5 validation](macos_hdf5_h5_generic_offline_validation.md) | 播放中打开另一文件、不同 geometry、失败回滚 | M5 | playback state 同步是潜在风险，需要运行时验证 |
