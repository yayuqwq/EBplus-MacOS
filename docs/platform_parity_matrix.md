...
| GUI-002 | GUI | Main event display | Wired into application | Requires verification | In progress | 用户观察到 idle 2D/OpenGL placeholder 正常；AppleMetalOpenGLRenderer 已映射；见 [M4A validation](macos_milestone_4a_validation.md) | live 与 RAW 帧显示、正确性和性能 | M4/M5/M6 | idle placeholder 不证明 event-data rendering |
| GUI-003 | GUI | Menus | Wired into application | Requires verification | In progress | 用户观察到 File、View、Theme、Tools、Help 五个菜单且无 Camera 菜单，并只读打开菜单；见 [M4A validation](macos_milestone_4a_validation.md) | 检查每个 action、快捷键和 modal dialog | M4 | 菜单位于 CustomTitleBar；未验证 action workflow |
...
| GUI-014 | GUI | Theme and layout persistence | Wired into application | Requires verification | In progress | 用户将 pink 主题切换后，isolated rerun 自动恢复；横向和纵向 resize 通过；观察到 repository-local layout.json；见 [M4A validation](macos_milestone_4a_validation.md) | 重启持久化、损坏文件和 macOS 路径 | M4 | theme persistence 是用户观察证据，不由 layout.json 单独推断；复杂 layout workflow 未验证 |
| GUI-015 | GUI | Clean exit and resource release | Implemented in source | Requires verification | In progress | idle 无相机状态下，关闭按钮和 Cmd+Q 均正常退出，实际 GUI exit code 为 0；见 [M4A validation](macos_milestone_4a_validation.md) | live、RAW、export、recording、algo window 并发退出 | M4-M7 | 未验证 active camera、RAW、export 或 algorithm window teardown |
