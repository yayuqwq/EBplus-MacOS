...
| BLD-003 | Build | Qt6 discovery | Implemented in source | Requires verification | Verified | Qt 6.9.3 discovery、compile 和 static Mach-O linkage audit 通过；见 [M3 validation](macos_milestone_3_validation.md) | Qt Cocoa application launch 与 Linux Qt platform plugin 仍需验证 | M3 | 仅 discovery/build 已验证；未运行 `gui_for_openeb`，Cocoa runtime 未验证 |
...
| BLD-006 | Build | GTest discovery | Implemented in source | Requires verification | Verified | GTest 1.17.0；9/9 discovery targets、295 tests discovered、295/295 CTest passed；见 [M3 validation](macos_milestone_3_validation.md) | Native Linux discovery/CTest 仍需验证 | M3 | CTest parallelism 2；test artifacts 位于 build tree 且最终无残留 |
...
| BLD-008 | Build | Main executable target | Implemented in source | Requires verification | Verified | `gui_algo`、`gui_core`、`gui_for_openeb` clean build 通过；production executable 为 non-fat arm64；见 [M3 validation](macos_milestone_3_validation.md) | GUI launch/runtime 仍需在 M4 验证 | M3 | executable 仅完成 build/install/static audit，未执行 |
...
| BLD-010 | Runtime integration | Dynamic library resolution | Implemented in source | Requires verification | In progress | EBplus build Mach-O 使用 repository-local OpenEB 5.2 与 Homebrew Qt/OpenCV；installed `LC_RPATH` 精确为 `@executable_path/../lib`；见 [M3 validation](macos_milestone_3_validation.md) | 执行 installed binary，并验证 dependency staging、portable closure 和 ONNX linkage | M3/M8 | M3 static linkage/RPATH portion 已验证；standalone loader closure 与 runtime 未验证 |
