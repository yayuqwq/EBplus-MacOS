...
| BLD-004 | Build | MetavisionSDK 5.2.0 discovery | Implemented in source | Requires verification | Not started | `find_package(MetavisionSDK 5.2.0 REQUIRED COMPONENTS base core stream)` | 验证隔离 prefix 被准确选中 | M2/M3 | `gui_core` 在 target 存在时额外链接 `MetavisionSDK::hal` |
...
| BLD-010 | Runtime integration | Dynamic library resolution | Implemented in source | Requires verification | Not started | `run.sh` 设置 `LD_LIBRARY_PATH`；`algo/CMakeLists.txt` 设置 ONNX RPATH | Linux `ldd` 与 macOS `otool -L` 检查 | M3/M8 | STATIC `gui_algo` 上 RPATH 是否传递到最终 executable 是潜在风险，需要运行时验证 |
| BLD-011 | Runtime integration | HAL plugin discovery | Wired into application | Requires verification | Not started | `gui/main.cpp::ensure_openeb_env_defaults()` 和 `run.sh` 设置 `MV_HAL_PLUGIN_PATH` | 枚举并打开真实设备，确认加载的 plugin | M2/M6 | 默认写死 `/usr/local/lib/metavision/hal/plugins`，macOS 路径需隔离配置 |
| BLD-012 | Runtime integration | HDF5 plugin discovery | Wired into application | Requires verification | Not started | `gui/main.cpp` 和 `run.sh` 设置 `HDF5_PLUGIN_PATH` | 打开、转换和导出 HDF5 | M2/M5/M7 | 默认使用 `/usr/local/lib/hdf5/plugin` |
...
| PKG-002 | Packaging | macOS RPATH | Not applicable | Not applicable | Not started | 当前只有 Linux 风格 `$ORIGIN/../lib` ONNX RPATH | `otool -l`、隔离 prefix、bundle launch | M3/M8 | 需使用 macOS loader path 语义 |
